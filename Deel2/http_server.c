...
void setHeader(char httpHeader[])
{
...
void dataNaarClient (int clientSocket){
        int inputs[3];
        ...
        for(int i = 0; i < 3 ; i++ ){
            if(i == 0){
                sprintf(test, "<p>Input 27: %d </p>", inputs[0]);
            }
            if(i == 1){
                sprintf(test, "<p>Input 22: %d </p>", inputs[1]);
            }
            if(i == 2){
                sprintf(test, "<p>Input 26: %d </p>", inputs[2]);
            }
...
int main(void)
{
    ...
    setHeader(httpHeader);  // Custom function to set header
    int clientSocket;
